...
var FileSystemStore = /** @class */ (function() {
  ...
  store5.gc().catch(err => {
    console.error(err)
      ...
      ```
     */
  FileSystemStore.prototype.gc = function() {
    var _this = this
    var files
    try {
    ...
    return Promise.all(
      files
        .filter(function(filename) {
          return path.extname(filename) === '.ttl'
        })
        .map(function(filename) {
          return new Promise(function(resolve, reject) {
            try {
              var ttl = JSON.parse(
                String(
                  _this.options.fs.readFileSync(
                    path.join(_this.options.path, filename)
                  )
                )
              )
              if (typeof ttl !== 'number' || ttl < Date.now()) {
                var key_1 = path.basename(filename, '.ttl')
                _this
                  .remove(key_1)
                  .then(function() {
                    resolve(key_1)
